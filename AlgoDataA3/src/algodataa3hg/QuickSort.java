...
public class QuickSort {
     ...
     */ 
    private void sort(Integer[] a, String[] b, int lo, int hi) { 
        if (hi <= lo) return; //if only one element left, consider it sorted
        
        int j = partition(a, b, lo, hi); //select new pivot
        //printArray(b);
        sort(a, b, lo, j-1);
        sort(a, b, j+1, hi);
    }
     ...
     * 
     */
    private void swap(Integer[] a, String []b, int i, int j) {
        int swap = a[i];
        String swapb = b[i];
     ...
     */
    private int partition(Integer[] a, String[] b, int lo, int hi){
     ...
     */
    public long sort(Integer[] a, String[] b) {
